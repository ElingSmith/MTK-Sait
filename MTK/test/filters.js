 const filterConfig = {
    ...
    const currentSubcat = "dorozhno_stroitelnaya";
    const currentSubsubcat = "Backhoe_Loader";
...
document.getElementById("apply-filters").addEventListener("click", () => {
    ...
    for (const [key, control] of Object.entries(sliders)) {
        if (control.noUiSlider) {
            ...
            // Если ничего не выбрано, добавляем все опции по умолчанию
            if (selectedOptions.length === 0) {
                filters[key] = control.map((checkbox) => checkbox.value); // Все опции
            } else {
